...
**Image tags.** `:latest-cuda` / `:latest-rocm` float to the newest build of each accelerator. `:cuda-vX.Y.Z` floats to the newest build of that vLLM version on CUDA. For reproducible deployments, pin to an immutable per-build tag like `:cuda-v0.27.1-1` or `:rocm-v0.27.1-1`. See [CLAUDE.md](./CLAUDE.md#building) for the full tag scheme.
...
# CUDA
docker build \
  --build-arg BASE_IMAGE=vllm/vllm-openai:v0.27.1 \
  --build-arg ACCEL=cuda \
  -t ghcr.io/tomaskir/vllm-wg-dockerized:cuda-v0.27.1-1 .

# ROCm
docker build \
  --build-arg BASE_IMAGE=vllm/vllm-openai-rocm:v0.27.1 \
  --build-arg ACCEL=rocm \
  -t ghcr.io/tomaskir/vllm-wg-dockerized:rocm-v0.27.1-1 .
```
